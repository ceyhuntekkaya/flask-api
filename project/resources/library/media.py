from flask.views import MethodView
from flask_smorest import Blueprint, abort
from sqlalchemy.exc import SQLAlchemyError, IntegrityError

from flask_jwt_extended import jwt_required

from db import db
from project.models.library.media import MediaModel
from project.schemas.library.media import MediaSchema

blp = Blueprint("Media", "media", description="Operations on media")


@blp.route("/media/<string:item_id>")
class WithId(MethodView):
    @jwt_required()
    @blp.response(200, MediaSchema)
    def get(self, item_id):
        item = MediaModel.query.get_or_404(item_id)
        return item

    @jwt_required()
    def delete(self, item_id):
        item = MediaModel.query.get_or_404(item_id)
        db.session.delete(item)
        db.session.commit()
        return {"message": "media deleted"}, 200

    @blp.arguments(MediaSchema)
    @blp.response(201, MediaSchema)
    def put(self, item_data, item_id):
        item = MediaModel.query.get(item_id)
        if item:
            item.price = item_data["price"]
            item.name = item_data["name"]
        else:
            item = MediaModel(id=item_id, **item_data)
        db.session.add(item)
        db.session.commit()

        return item


@blp.route("/media")
class Plain(MethodView):
    @jwt_required()
    @blp.response(200, MediaSchema(many=True))
    def get(self):
        return MediaModel.query.all()

    @jwt_required(fresh=True)
    @blp.arguments(MediaSchema)
    @blp.response(201, MediaSchema)
    def post(self, item_data):
        item = MediaModel(**item_data)
        try:
            db.session.add(item)
            db.session.commit()
        except IntegrityError:
            abort(
                400,
                message="A media with that name already exists.",
            )
        except SQLAlchemyError:
            abort(500, message="An error occurred creating the media.")

        return item
